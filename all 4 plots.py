...
#Data uploaded below
d1 = pd.read_csv('arxiv_pearson_normal.csv')
d2 = pd.read_csv('arxiv_spearman_normal.csv')
d3 = pd.read_csv('pubmed_pearson_normal.csv')
d4 = pd.read_csv('pubmed_spearman_normal.csv')
...
sns.heatmap(data1, annot= True, cmap = 'vlag', cbar= False, xticklabels= True
            , yticklabels=True, linewidth = 0.5, vmin = -1, vmax = 1, fmt = '.2f',annot_kws={'size' : 16}, ax=ax1)

sns.heatmap(data2, annot= True, cmap = 'vlag', cbar= False, xticklabels= True
            , yticklabels=True, linewidth = 0.5, vmin = -1, vmax = 1, fmt = '.2f',annot_kws={'size' : 16}, ax=ax2)

sns.heatmap(data3,annot= True, cmap = 'vlag', cbar= False,xticklabels= True
            , yticklabels=True, linewidth = 0.5, vmin = -1, vmax = 1, fmt = '.2f',annot_kws={'size' : 16}, ax=ax3)

sns.heatmap(data4,annot= True, cmap = 'vlag', cbar= False, xticklabels= True
            , yticklabels=True, linewidth = 0.5, vmin = -1, vmax = 1, fmt = '.2f', annot_kws={'size' : 16}, ax=ax4)

sns.heatmap(np.zeros((1,1)), cmap="vlag", cbar = True, vmin = -1, vmax = 1, cbar_kws = dict(use_gridspec = False, location = 'bottom'), ax=ax5)
ax5.remove()

# save the combined heatmaps to an SVG file
fig.savefig("heatmaps_normal.png")
